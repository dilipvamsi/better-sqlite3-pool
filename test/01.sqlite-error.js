...
describe("SqliteError", function () {
  ...
  it("should accept two arguments for setting the message and error code", function () {
    const err = new SqliteError("foobar", "baz");
    expect(err.message).to.equal("foobar");
    expect(err.code).to.equal("baz");
    expect(new SqliteError(123, "baz").message).to.equal("123");
    expect(() => new SqliteError("foo")).to.throw(TypeError);
    expect(() => new SqliteError("foo", 123)).to.throw(TypeError);
  });
  it("should capture stack traces", function () {
    expect(new SqliteError(null, "baz").stack).to.be.a("string");
  });
});
